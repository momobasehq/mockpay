...
curl -s http://localhost:8080/ | grep -q "Simulation configuration"
echo "UI: available"
...
curl -s -o /dev/null -X POST "http://localhost:8080/mtn/collection/v1_0/requesttopay" \
  -H "Authorization: Bearer $COLLECTION_TOKEN" \
...
curl -s -o /dev/null -X POST "http://localhost:8080/mtn/disbursement/v1_0/transfer" \
...
curl -s -o /dev/null -X POST "http://localhost:8080/airtel/merchant/v2/payments/" \
  -H "Authorization: Bearer $AIRTEL_TOKEN" \
  -H "Content-Type: application/json" \
...
curl -s -X POST "http://localhost:8080/airtel/standard/v1/disbursements/" \
  -H "Authorization: Bearer $AIRTEL_TOKEN" \
  -H "Content-Type: application/json" \
